...
/*     */       } 
/*     */       
/* 620 */       int pattern = CODEWORD_TABLE[cluster][left];
/* 621 */       encodeChar(pattern, 17, logic.getCurrentRow());
/*     */       
/* 623 */       for (int x = 0; x < c; x++) {
/* 624 */         pattern = CODEWORD_TABLE[cluster][fullCodewords.charAt(idx)];
/* 625 */         encodeChar(pattern, 17, logic.getCurrentRow());
/* 626 */         idx++;
/*     */       } 
/*     */       
/* 629 */       if (this.compact) {
...
/*     */       } else {
/* 632 */         pattern = CODEWORD_TABLE[cluster][right];
/* 633 */         encodeChar(pattern, 17, logic.getCurrentRow());
/*     */         
...
/* 649 */     String highLevel = PDF417HighLevelEncoder.encodeHighLevel(msg, this.compaction, this.encoding);
/* 650 */     int sourceCodeWords = highLevel.length();
/*     */     
/* 652 */     int[] dimension = determineDimensions(sourceCodeWords, errorCorrectionCodeWords);
/*     */     
/* 654 */     int cols = dimension[0];
...
/* 661 */       throw new WriterException("Encoded message contains to many code words, message to big (" + msg
/* 662 */           .length() + " bytes)");
/*     */     }
/* 664 */     int n = sourceCodeWords + pad + 1;
/* 665 */     StringBuilder sb = new StringBuilder(n);
/* 666 */     sb.append((char)n);
/* 667 */     sb.append(highLevel);
/* 668 */     for (int i = 0; i < pad; i++) {
/* 669 */       sb.append('΄');
/*     */     }
/* 671 */     String dataCodewords = sb.toString();
/*     */ 
/*     */     
/* 674 */     String ec = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords, errorCorrectionLevel);
/* 675 */     String fullCodewords = dataCodewords + ec;
/*     */ 
/*     */     
/* 678 */     this.barcodeMatrix = new BarcodeMatrix(rows, cols);
/* 679 */     encodeLowLevel(fullCodewords, cols, rows, errorCorrectionLevel, this.barcodeMatrix);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 691 */     float ratio = 0.0F;
/* 692 */     int[] dimension = null;
/*     */     
/* 694 */     for (int cols = this.minCols; cols <= this.maxCols; cols++) {
/*     */       
/* 696 */       int rows = calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, cols);
/*     */       
/* 698 */       if (rows < this.minRows) {
/*     */         break;
/*     */       }
/*     */       
/* 702 */       if (rows <= this.maxRows) {
/*     */ 
/*     */ 
/*     */         
/* 706 */         float newRatio = (17 * cols + 69) * 0.357F / rows * 2.0F;
/*     */ 
/*     */         
/* 709 */         if (dimension == null || Math.abs(newRatio - 3.0F) <= Math.abs(ratio - 3.0F)) {
/*     */ 
/*     */ 
/*     */           
/* 713 */           ratio = newRatio;
/* 714 */           dimension = new int[] { cols, rows };
/*     */         } 
/*     */       } 
/*     */     } 
/* 718 */     if (dimension == null) {
/* 719 */       int rows = calculateNumberOfRows(sourceCodeWords, errorCorrectionCodeWords, this.minCols);
/* 720 */       if (rows < this.minRows) {
/* 721 */         dimension = new int[] { this.minCols, this.minRows };
/*     */       }
/*     */     } 
/*     */     
/* 725 */     if (dimension == null) {
/* 726 */       throw new WriterException("Unable to fit message in columns");
/*     */     }
/*     */     
/* 729 */     return dimension;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 741 */     this.maxCols = maxCols;
/* 742 */     this.minCols = minCols;
/* 743 */     this.maxRows = maxRows;
/* 744 */     this.minRows = minRows;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 751 */     this.compaction = compaction;
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 765 */     this.encoding = encoding;
/*     */   }
/*     */ }


/* Location:              C:\Users\carlos\Documents\InnoExtractor\Cyber Password Manager 1_0\{app}\cpm_1.0.jar!\com\google\zxing\pdf417\encoder\PDF417.class
