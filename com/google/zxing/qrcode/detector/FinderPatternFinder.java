...
/*     */ 
/*     */     
/*  88 */     int iSkip = 3 * maxI / 228;
/*  89 */     if (iSkip < 3 || tryHarder) {
/*  90 */       iSkip = 3;
/*     */     }
/*     */     
...
/* 114 */                 boolean confirmed = handlePossibleCenter(stateCount, i, j, pureBarcode);
/* 115 */                 if (confirmed) {
/*     */ 
/*     */                   
/* 118 */                   iSkip = 2;
...
/* 169 */         boolean confirmed = handlePossibleCenter(stateCount, i, maxJ, pureBarcode);
/* 170 */         if (confirmed) {
...
/* 181 */     ResultPoint.orderBestPatterns((ResultPoint[])patternInfo);
/*     */     
...
/* 209 */       return false;
/*     */     }
/* 211 */     float moduleSize = totalModuleSize / 7.0F;
/* 212 */     float maxVariance = moduleSize / 2.0F;
/*     */     
/* 214 */     return (
/* 215 */       Math.abs(moduleSize - stateCount[0]) < maxVariance && 
...
/* 219 */       Math.abs(moduleSize - stateCount[4]) < maxVariance);
/*     */   }
/*     */   
...
/*     */ 
/*     */     
/* 316 */     int stateCountTotal = stateCount[0] + stateCount[1] + stateCount[2] + stateCount[3] + stateCount[4];
/* 317 */     return (
/* 318 */       Math.abs(stateCountTotal - originalStateCountTotal) < 2 * originalStateCountTotal && 
/* 319 */       foundPatternCross(stateCount));
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 335 */     BitMatrix image = this.image;
/*     */     
/* 337 */     int maxI = image.getHeight();
...
/* 407 */     BitMatrix image = this.image;
/*     */     
/* 409 */     int maxJ = image.getWidth();
...
/* 491 */     float centerI = crossCheckVertical(i, (int)centerJ, stateCount[2], stateCountTotal);
/* 492 */     if (!Float.isNaN(centerI)) {
/*     */       
/* 494 */       centerJ = crossCheckHorizontal((int)centerJ, (int)centerI, stateCount[2], stateCountTotal);
/* 495 */       if (!Float.isNaN(centerJ) && (!pureBarcode || 
...
/* 500 */           FinderPattern center = this.possibleCenters.get(index);
/*     */           
/* 502 */           if (center.aboutEquals(estimatedModuleSize, centerI, centerJ)) {
...
/* 528 */     int max = this.possibleCenters.size();
/* 529 */     if (max <= 1) {
/* 530 */       return 0;
/*     */     }
/* 532 */     ResultPoint firstConfirmedCenter = null;
/* 533 */     for (FinderPattern center : this.possibleCenters) {
/* 534 */       if (center.getCount() >= 2) {
...
/*     */         
/* 545 */         return (int)(Math.abs(firstConfirmedCenter.getX() - center.getX()) - Math.abs(firstConfirmedCenter.getY() - center.getY())) / 2;
/*     */       } 
/*     */     } 
/*     */     
/* 549 */     return 0;
...
/* 561 */     for (FinderPattern pattern : this.possibleCenters) {
/* 562 */       if (pattern.getCount() >= 2) {
/* 563 */         confirmedCount++;
...
/* 590 */     int startSize = this.possibleCenters.size();
/* 591 */     if (startSize < 3)
/*     */     {
...
/* 601 */       for (FinderPattern center : this.possibleCenters) {
/* 602 */         float size = center.getEstimatedModuleSize();
...
/* 614 */         FinderPattern pattern = this.possibleCenters.get(i);
/* 615 */         if (Math.abs(pattern.getEstimatedModuleSize() - average) > limit) {
...
/* Location:              C:\Users\carlos\Documents\InnoExtractor\Cyber Password Manager 1_0\{app}\cpm_1.0.jar!\com\google\zxing\qrcode\detector\FinderPatternFinder.class
